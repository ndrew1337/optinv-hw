...
<body>
  ...
  <ul>
    ...
    <li>Межбиржевой арбитраж даёт прибыль при расхождении цен между Binance/Bybit/OKX.</li>
    <li>С комиссиями используется лаг 1 бар на стороне покупки (котировки обновляются не синхронно).</li>
    <li>Без комиссий — синхронный снимок close (верхняя оценка).</li>
    <li>Для реальной торговли нужны order book, latency и риск задержки вывода.</li>
  </ul>
</body>
</html>"""
